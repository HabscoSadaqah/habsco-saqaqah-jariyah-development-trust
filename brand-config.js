...
(function(){
  ...
  const brand=financePages.has(path)?{
    legal:'Habsco Free Interest Multipurposes Cooperative Society',
    service:'Habsco Finance',
    descriptor:'Savings, cooperative finance and interest-free financial services',
    type:'finance'
  }:businessPages.has(path)?{
    legal:'Habsco Universal Enterprises',
    service:'Habsco Universal Enterprises',
    descriptor:'Business and commercial services',
    type:'business'
  }:{
    legal:'Habsco Sadaqah Jariyah Development Trust',
    service:'Habsco Sadaqah Jariyah Development Trust',
    descriptor:'Continuous charity, Waqf and community development',
    type:'charity'
  };
  ...
  function setFooter(){
    ...
    if(fb){const p=fb.querySelector('p'); if(brand.type==='finance'){
      const img=fb.querySelector('img'); if(img)img.alt=brand.service;
      let h=fb.querySelector('h3'); if(h)h.textContent=brand.legal;
      if(p)p.textContent=brand.service+' is the digital finance platform of '+brand.legal+'.';
    }}
  }
  function addGovernanceBadge(){
    if(path==='index.html'){
      ...
      if(main&&!document.getElementById('habsco-ecosystem')){
        ...
        main.insertBefore(sec,document.querySelector('footer'));
  ...
  function init(){
    ...
    if(path==='statement.html'){
      ...
      const p=document.querySelector('.hero p');if(p&&p.id!=='memberInfo')p.insertAdjacentHTML('afterend','');
