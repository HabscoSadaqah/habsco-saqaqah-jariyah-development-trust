...
(function(){
  ...
  function setFooter(){
    ...
    if(fb&&brand.type==='finance'){
      ...
      const p=fb.querySelector('p');if(p)p.textContent='Cooperative Finance services by '+brand.legal+'.';
    }
  ...
  function addGovernanceBadge(){
    ...
    if(main&&!document.getElementById('habsco-ecosystem')){
      ...
      sec.innerHTML='<div class="habsco-ecosystem-inner"><div class="eyebrow">THE HABSCO ECOSYSTEM</div><h2>One Habsco ecosystem. Three clear purposes.</h2><p class="ecosystem-intro">Habsco operates through three distinct entities. Each service is presented under the entity responsible for it, while members can enjoy one consistent digital experience.</p><div class="ecosystem-grid"><article><strong>Habsco Universal Enterprises</strong><span>BUSINESS · Buying &amp; selling, utility and commercial services</span></article><article><strong>Habsco Free Interest Multipurposes Cooperative Society</strong><span>FINANCE · Cooperative savings, shares, interest-free loans and related financial services</span></article><article><strong>Habsco Sadaqah Jariyah Development Trust</strong><span>CHARITY · Donations, Sadaqah Jariyah, Waqf and continuous community development</span></article></div></div>';
  ...
  function init(){
    ...
    if(path==='finance.html'){
      document.title='Finance | Habsco Free Interest Multipurposes Cooperative Society';
      document.querySelectorAll('.finance-mini-badge').forEach(el=>el.innerHTML='<i></i> HABSCO COOPERATIVE FINANCE');
