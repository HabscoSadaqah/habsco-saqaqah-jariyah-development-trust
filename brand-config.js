...
(function(){
  ...
  const brand=financePages.has(path)?{legal:'Habsco Free Interest Multipurposes Cooperative Society',service:'Habsco Free Interest Multipurposes Cooperative Society',descriptor:'Cooperative savings, shares, interest-free loans and financial services',type:'finance'}:businessPages.has(path)?{legal:'Habsco Universal Enterprises',service:'Habsco Universal Enterprises',descriptor:'Business, commercial and utility services',type:'business'}:{legal:'Habsco Sadaqah Jariyah Development Trust',service:'Habsco Sadaqah Jariyah Development Trust',descriptor:'Continuous charity, Waqf and community development',type:'charity'};
  ...
  function setFooter(){
    ...
    if(fb&&brand.type==='finance'){
      ...
      const p=fb.querySelector('p');if(p)p.textContent='Cooperative savings, shares, interest-free loans and financial services by '+brand.legal+'.';
    }
  ...
  function addGovernanceBadge(){
    ...
    if(main&&!document.getElementById('habsco-ecosystem')){
      ...
      sec.innerHTML='<div class="habsco-ecosystem-inner"><div class="eyebrow">THE HABSCO ECOSYSTEM</div><h2>One Habsco ecosystem. Clear financial and community purposes.</h2><p class="ecosystem-intro">Habsco organisations manage resources in different ways, with each entity responsible for a clearly defined purpose. The website is led by Habsco Sadaqah Jariyah Development Trust, while Finance, Business and Charity services are presented under their responsible entities.</p><div class="ecosystem-grid"><article><strong>Habsco Universal Enterprises</strong><span>Business, buying &amp; selling, utility and commercial services</span></article><article><strong>Habsco Free Interest Multipurposes Cooperative Society</strong><span>Cooperative savings, shares, interest-free loans and related financial services</span></article><article><strong>Habsco Sadaqah Jariyah Development Trust</strong><span>Charity, donations, Waqf &amp; continuous community development — the organisation behind this website and domain</span></article></div></div>';
  ...
  function loadMemberEcosystemUI(){
    ...
    const s=document.createElement('script');s.src='/habsco-ecosystem-ui.js?v=20260913-1';s.dataset.habscoEcosystemUi='1';document.head.appendChild(s);
  }
  function init(){
    ...
    if(path==='finance.html'){
      document.title='Habsco Free Interest Multipurposes Cooperative Society | Finance';
      document.querySelectorAll('.finance-mini-badge').forEach(el=>el.innerHTML='<i></i> HABSCO COOPERATIVE');
      const tagline=document.querySelector('.finance-hero-tagline');if(tagline)tagline.textContent='Cooperative savings, shares, interest-free loans and financial services.';
    }
    if(path==='statement.html'){
      document.title='Habsco Free Interest Multipurposes Cooperative Society | Account Statement';
      const b=document.querySelector('.brand');if(b)b.textContent='HABSCO COOPERATIVE';
