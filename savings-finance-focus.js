...
`;
 ...
 if(!modal.dataset.scrollGuard){modal.dataset.scrollGuard='1';const stop=e=>{if(!e.target.closest('.hf-savings-sheet')){e.preventDefault();e.stopPropagation();}};modal.addEventListener('touchmove',stop,{passive:false});modal.addEventListener('wheel',stop,{passive:false});}
};
const start=()=>{if(window.__hfSavingsDashboardObserver)return;window.__hfSavingsDashboardObserver=new MutationObserver(()=>{const modal=document.getElementById('hfSavingsModal');if(modal)polish();else{document.documentElement.classList.remove('hf-savings-locked');document.body.classList.remove('hf-savings-locked');}});window.__hfSavingsDashboardObserver.observe(document.body,{childList:true,subtree:true});if(document.getElementById('hfSavingsModal'))polish();};
