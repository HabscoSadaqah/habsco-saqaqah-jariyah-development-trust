...
const polish=()=>{
 ...
 if(!sheet)return;
 if(modal.dataset.financeFocus==='1')return;
 ...
 if(balance&&!sheet.querySelector('.hf-finance-focus-label'))balance.insertAdjacentHTML('afterend','<div class="hf-finance-focus-label">FINANCE OVERVIEW</div>');
 ...
 if(progress&&!progress.querySelector('.hf-cycle-mark')){
   ...
   const m=(first?.textContent||'').match(/(\d+)\s+(?:day|days|week|weeks|month|months)/i),g=goalText.match(/(\d+)/);
   const saved=Math.max(0,Number(m?.[1]||0)),goal=Math.max(1,Number(g?.[1]||6));
   const track=progress.querySelector('i');
   if(track){track.classList.add('hf-cycle-track');const marks=Math.min(goal,26);for(let i=1;i<=marks;i++){const mark=document.createElement('span');mark.className='hf-cycle-mark'+(i<=saved?' is-paid':'');mark.style.left=((i/goal)*100)+'%';mark.title=i<=saved?`Paid cycle ${i}`:`Cycle ${i}`;track.appendChild(mark)}track.setAttribute('aria-label',`${saved} of ${goal} paid cycles completed`)}
 }
 ...
 .hf-savings-sheet{position:absolute!important;left:50%!important;top:50%!important;bottom:auto!important;right:auto!important;width:min(560px,calc(100vw - 28px))!important;max-height:calc(100vh - 30px)!important;overflow-y:auto!important;overflow-x:hidden!important;box-sizing:border-box!important;background:linear-gradient(145deg,#fff 0%,#f8fcfa 58%,#edf5f0 100%);border:1px solid rgba(255,255,255,.95);border-radius:22px;box-shadow:0 34px 90px rgba(1,39,24,.34),0 8px 24px rgba(1,39,24,.12),inset 0 1px 0 #fff;transform:translate(-50%,-50%)!important;}
 .hf-savings-backdrop{position:absolute!important;inset:0!important;background:rgba(4,25,16,.55)!important;backdrop-filter:blur(6px)!important}
 .hf-savings-sheet:before{content:'';position:absolute;inset:0;border-radius:22px;pointer-events:none;background:linear-gradient(120deg,rgba(255,255,255,.65),transparent 28%,transparent 72%,rgba(8,116,67,.035));z-index:0}
 .hf-savings-sheet>*{position:relative;z-index:1}
 .hf-savings-close{z-index:5!important;display:flex!important;align-items:center;justify-content:center;width:42px!important;height:42px!important;right:12px!important;top:12px!important;background:linear-gradient(145deg,#fff,#e7efea)!important;color:#174b35!important;font-size:17px!important;font-weight:900!important;box-shadow:0 5px 14px rgba(0,0,0,.12),inset 0 1px 0 #fff;cursor:pointer;transition:transform .18s ease,box-shadow .18s ease,background .18s ease}.hf-savings-close:hover{background:#087443!important;color:#fff!important;transform:translateY(-1px) scale(1.03)}.hf-savings-close:active{transform:scale(.95)}
 .hf-savings-eyebrow{color:#087443!important;letter-spacing:1.4px!important}.hf-finance-focus-label{margin:13px 0 7px;font-size:7px;letter-spacing:1.1px;font-weight:900;color:#7b8982}.hf-savings-grid.hf-finance-snapshot{margin-top:0;grid-template-columns:1fr 1fr}.hf-savings-grid.hf-finance-snapshot>div{background:linear-gradient(145deg,#fbfdfc,#f1f7f3);border-color:#dfe9e3;padding:12px;box-shadow:inset 0 1px 0 #fff,0 4px 12px rgba(12,57,39,.05)}.hf-savings-grid.hf-finance-snapshot>div:first-child{border-left:3px solid #087443}.hf-savings-grid.hf-finance-snapshot>div:last-child{border-left:3px solid #c59a4b}
 .hf-finance-section-title{display:flex!important;justify-content:space-between!important;align-items:center;margin:14px 0 7px;color:#17221c!important;font-size:10px!important}.hf-finance-section-title span{font-size:7px;color:#849189;font-weight:700}.hf-savings-progress{margin-top:0}.hf-savings-plan{background:linear-gradient(145deg,#f9fcfa,#eef5f1);border-color:#dce8e1;box-shadow:inset 0 1px 0 #fff,0 6px 16px rgba(12,57,39,.045)}.hf-finance-secondary{background:#fffaf0;border-color:#eadfc9}
 .hf-savings-progress i.hf-cycle-track{position:relative;box-shadow:inset 0 2px 4px rgba(1,39,24,.10),0 1px 0 #fff}.hf-savings-progress i.hf-cycle-track em{position:relative;z-index:1;box-shadow:0 1px 5px rgba(8,116,67,.34)}.hf-cycle-mark{position:absolute;z-index:2;top:50%;width:5px;height:5px;border-radius:50%;transform:translate(-50%,-50%);background:#cbd7d0;border:1px solid rgba(255,255,255,.9);box-shadow:0 1px 2px rgba(0,0,0,.16)}.hf-cycle-mark.is-paid{background:#fff;box-shadow:0 0 0 2px rgba(8,116,67,.22),0 1px 5px rgba(0,0,0,.18)}
 .hf-rules-head{display:flex;align-items:center;justify-content:space-between;gap:10px}.hf-rules-head strong{font-size:10px!important;color:#17221c}.hf-rules-toggle{border:1px solid #dce5df;background:#fff;color:#087443;border-radius:8px;padding:5px 9px;font-size:7px;font-weight:900;cursor:pointer;box-shadow:0 2px 7px rgba(0,0,0,.05)}.hf-rules-body p{margin:6px 0 0!important}.hf-rules-body.is-collapsed{display:none}.hf-finance-actions{grid-template-columns:1fr 1fr!important}.hf-finance-actions a.primary,.hf-finance-actions button.primary{box-shadow:0 8px 18px rgba(8,116,67,.18)}
 @media(max-width:600px){.hf-savings-sheet{left:50%!important;top:50%!important;width:calc(100vw - 18px)!important;max-height:calc(100vh - 24px)!important;border-radius:20px!important;padding:16px!important;padding-top:20px!important;transform:translate(-50%,-50%)!important}.hf-finance-actions{grid-template-columns:1fr!important}.hf-savings-close{width:44px!important;height:44px!important;right:9px!important;top:9px!important}}
