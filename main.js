...
(() => {
  ...
  const seo = {
    ...
    contact: ['Contact Us | Habsco Sadaqah Jariyah Development Trust','Contact Habsco Sadaqah Jariyah Development Trust for enquiries, volunteering, partnerships and community project information.'],
    donate: ['Donate | Habsco Sadaqah Jariyah Development Trust','Support Habsco Sadaqah Jariyah Development Trust and help build lasting community benefit through Sadaqah Jariyah projects.']
  };
  const data = seo[page];
  if (data) {
  ...
  if (isPublicPage) {
    const applyPublicShell=()=>{
      ...
      if(!document.querySelector('link[data-app-css]')){const css=document.createElement('link');css.rel='stylesheet';css.href='app.css?v=home-shell-5';css.dataset.appCss='true';document.head.appendChild(css)}
      let nav=document.querySelector('.app-bottom-nav');
      if(!nav){
        nav=document.createElement('nav');nav.className='app-bottom-nav';nav.setAttribute('aria-label','Primary navigation');
        nav.innerHTML=[['index.html','⌂','Home','index'],['impact.html','◈','Impact','impact'],['finance.html','₦','Finance','finance'],['contact.html','●','Contact','contact'],['donate.html','♡','Donate','donate']].map(([href,icon,label,key])=>`<a href="${href}" class="${page===key?'active ':''}${key==='donate'?'donate-tab':''}"><span aria-hidden="true">${icon}</span><small>${label}</small></a>`).join('');
        document.body.appendChild(nav);
      }
    };
    applyPublicShell();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',applyPublicShell,{once:true});window.addEventListener('load',applyPublicShell,{once:true});setTimeout(applyPublicShell,80);
  } else {
    const clearOtherPageShell=()=>{document.body?.classList.remove('public-app');document.querySelectorAll('header.logo-only-header,.app-bottom-nav').forEach(el=>el.remove())};
    clearOtherPageShell();if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',clearOtherPageShell,{once:true});window.addEventListener('load',clearOtherPageShell,{once:true});
  }
  ...
  const footerStyle=document.createElement('style');footerStyle.textContent=`footer .footer-brand,.programs-page footer .footer-brand,.programs-page footer .footer>div:first-child,footer .footer>div:first-child:has(>img){display:none!important}footer .footer{grid-template-columns:repeat(3,minmax(0,1fr))!important}.programs-page footer .footer{grid-template-columns:repeat(3,minmax(0,1fr))!important}@media(max-width:720px){footer .footer,.programs-page footer .footer{grid-template-columns:1fr 1fr!important}}@media(max-width:430px){footer .footer,.programs-page footer .footer{grid-template-columns:1fr!important}}.hero .actions a[href="auth.html"],.hero .actions .btn.primary[href="auth.html"]{position:relative;z-index:2;border:1px solid rgba(244,201,79,.9)!important;box-shadow:0 0 0 0 rgba(244,201,79,.72),0 8px 26px rgba(0,0,0,.16);animation:portalPulse 1.8s infinite,portalFloat 2.8s ease-in-out infinite;will-change:transform,box-shadow}.hero .actions a[href="auth.html"]:hover,.hero .actions .btn.primary[href="auth.html"]:hover{box-shadow:0 0 0 8px rgba(244,201,79,.14),0 12px 34px rgba(244,201,79,.28)!important}@keyframes portalPulse{0%,100%{box-shadow:0 0 0 0 rgba(244,201,79,.72),0 8px 26px rgba(0,0,0,.16)}50%{box-shadow:0 0 0 12px rgba(244,201,79,0),0 10px 30px rgba(244,201,79,.18)}}@keyframes portalFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}`;document.head.appendChild(footerStyle);
