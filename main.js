...
(() => {
  ...
  if (!document.querySelector('link[data-app-css]')) { const css=document.createElement('link'); css.rel='stylesheet'; css.href='app.css'; css.dataset.appCss='true'; document.head.appendChild(css); }
  const brand=document.querySelector('header .brand');
  if(brand){ brand.setAttribute('aria-label','Habsco Sadaqah Jariyah Development Trust'); brand.querySelector('.brand-name')?.remove(); }
  const toggle=document.querySelector('.mobile-toggle'); const menu=document.querySelector('.menu');
  if(toggle&&menu){toggle.setAttribute('aria-expanded','false');toggle.addEventListener('click',()=>{const open=menu.classList.toggle('open');toggle.setAttribute('aria-expanded',String(open));});}
  ...
  if(publicPages.includes(path)&&footer){footer.innerHTML='<div class="copyright">© 2026 Habsco Sadaqah Jariyah Development Trust. All Rights Reserved.</div>';footer.setAttribute('aria-label','Site footer');}
  ...
  footerStyle.textContent=`
    ...
    footer .copyright:before{content:'✦  ';color:#f4c94f;font-size:10px} footer .copyright:after{content:'  ✦';color:#f4c94f;font-size:10px}
