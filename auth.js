...
form.addEventListener('submit',async e=>{
  ...
  try{
    ...
    if(mode==='signup'){
      ...
      if(error)throw error;
      if(data?.session){setMessage('Account created. Opening your member portal…',true);setTimeout(()=>go('./member.html'),200);return;}
      setMessage('Registration successful. Check your email if confirmation is required, then log in.',true);
      submitBtn.disabled=false;submitBtn.textContent='CREATE MEMBER ACCOUNT';
      return;
    }
    ...
    try{
      ...
      const result=await Promise.race([profilePromise,new Promise(resolve=>setTimeout(()=>resolve({data:null,error:new Error('profile lookup timeout')}),1200))]);
      if(!result.error&&result.data?.role==='admin'&&result.data?.status==='active')destination='./admin.html';
