...
function position(){
 ...
 set(ps,{position:'fixed',left:rect.left+'px',top:rect.top+'px',right:'auto',bottom:'auto',transform:'none',width:rect.width+'px',height:rect.height+'px',maxWidth:rect.width+'px',maxHeight:rect.height+'px',boxSizing:'border-box',overflow:'auto',margin:'0',padding:'16px',borderRadius:'22px',background:'#fff',border:'1px solid #dce9e2',boxShadow:'0 20px 55px rgba(0,0,0,.28),0 5px 18px rgba(8,116,67,.12)',zIndex:'2',pointerEvents:'auto',opacity:'1',visibility:'visible'});
}
function run(){
 const modal=document.getElementById('hfSavingsModal');
 const sheet=modal?.querySelector('.hf-savings-sheet');
