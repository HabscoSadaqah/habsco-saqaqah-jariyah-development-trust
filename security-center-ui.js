...
function mount(){
 ...
 document.body.appendChild(sheet);sheet.querySelector(".hf-security-panel").appendChild(panel);
 ...
 const logout=document.getElementById("logout");top.insertBefore(trigger,logout||null);const addAdminButton=async()=>{try{const sb=window.supabase?.createClient?.("https://ythnoeyxovapydbmymdo.supabase.co","sb_publishable_nfSR2tMCFuHCpkOjjNIakw_P85zunsN");if(!sb)return;const {data:{session}}=await sb.auth.getSession();const email=(session?.user?.email||"").trim().toLowerCase();let admin=email==="habscosadaqah@gmail.com";try{const {data:v}=await sb.rpc("is_admin");admin=admin||v===true}catch{}if(!admin||document.getElementById("hfAdminControlTrigger"))return;const a=document.createElement("a");a.id="hfAdminControlTrigger";a.href="admin.html";a.title="Admin Control Center";a.setAttribute("aria-label","Open Admin Control Center");a.innerHTML="<span aria-hidden=\"true\">⚙️</span>";a.style.cssText="width:40px;height:40px;margin-right:6px;border:1px solid rgba(8,116,67,.16);background:#087443;color:#fff;border-radius:12px;display:grid;place-items:center;font-size:18px;text-decoration:none;font-weight:900";top.insertBefore(a,trigger)}catch{}};addAdminButton();setTimeout(addAdminButton,1200);
 const open=()=>sheet.classList.add("open"),close=()=>sheet.classList.remove("open");
 trigger.onclick=open;sheet.querySelector(".hf-security-backdrop").onclick=close;sheet.querySelector(".hf-security-close").onclick=close;document.addEventListener("keydown",e=>{if(e.key==="Escape")close()},{passive:true});
}
function boot(){
 if(!onMember())return;
