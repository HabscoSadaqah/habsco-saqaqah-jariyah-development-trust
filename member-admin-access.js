const HABSCO_ADMIN_URL='https://ythnoeyxovapydbmymdo.supabase.co';
const HABSCO_ADMIN_KEY='sb_publishable_nfSR2tMCFuHCpkOjjNIakw_P85zunsN';
(async()=>{
  try{
    if(!window.supabase?.createClient)return;
    const sb=window.supabase.createClient(HABSCO_ADMIN_URL,HABSCO_ADMIN_KEY);
    ...
    if(top&&!document.getElementById('memberAdminControl')){
    ...
    const dashboard=document.getElementById('hfAdminDashboard');
    if(dashboard){
      dashboard.classList.add('visible');
      dashboard.setAttribute('aria-hidden','false');
      const note=dashboard.querySelector('.hf-admin-note');
      if(note)note.textContent='Visible only to active administrators. Member features remain above this panel.';
    }
