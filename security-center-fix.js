(()=>{
  ...
  const loadSavingsPinFix=()=>{if(document.querySelector('script[data-hf-savings-pin-fix]'))return;const s=document.createElement('script');s.src='pin-board-fix.js?v=20260913-6';s.async=false;s.dataset.hfSavingsPinFix='1';document.head.appendChild(s)};
  const loadPinRefreshFix=()=>{if(document.querySelector('script[data-hf-pin-refresh-fix]'))return;const s=document.createElement('script');s.src='pin-refresh-fix.js?v=20260913-2';s.async=false;s.dataset.hfPinRefreshFix='1';document.head.appendChild(s)};
  ...
  loadPinRefreshFix();
  loadRoleFloatNav();
  const start=()=>{const sb=getClient(),panel=document.getElementById('hfSecurityCenter');if(!sb||!panel){setTimeout(start,250);return}applyMemberBranding();addBiometricCard();setTimeout(addBiometricCard,300);setTimeout(addBiometricCard,1000)};
