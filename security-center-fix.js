(()=>{
  ...
  const loadSavingsPinFix=()=>{if(document.querySelector('script[data-hf-savings-pin-fix]')||document.getElementById('hf-pin-board-fix-style'))return;const s=document.createElement('script');s.src='pin-board-fix.js?v=20260913-4';s.async=false;s.dataset.hfSavingsPinFix='1';s.onload=()=>{try{window.dispatchEvent(new Event('resize'))}catch(_){}};document.head.appendChild(s)};
  ...
  const start=()=>{const sb=getClient(),panel=document.getElementById('hfSecurityCenter');if(!sb||!panel){setTimeout(start,250);return}applyMemberBranding();loadSavingsPinFix();addBiometricCard();loadRoleFloatNav();setTimeout(addBiometricCard,300);setTimeout(addBiometricCard,1000)};
