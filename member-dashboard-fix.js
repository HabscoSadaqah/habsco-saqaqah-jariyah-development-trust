...
async function run(){
 ...
 let available=0,savings=0,shares=0,special=0;
 ...
 const {data:tx}=await sb.from('transactions').select('reference,type,amount,direction,description,status,created_at').eq('user_id',uid).order('created_at',{ascending:false}).limit(20);
 window.__hassanLatestTx=tx||[];
 const list=document.getElementById('recentFundingList');
 if(list){list.classList.add('hassan-compact-transactions');list.innerHTML=(tx||[]).slice(0,8).map(t=>{const credit=String(t.direction||'').toLowerCase()==='credit',sign=credit?'+':'−',label=t.type==='funding'?'Wallet Funding':(t.description||t.type||'Transaction'),when=new Date(t.created_at).toLocaleDateString('en-NG',{day:'2-digit',month:'short'});return `<div class="hassan-tx-row"><div class="hassan-tx-main"><strong>${esc(label)}</strong><small>${esc(t.reference||'—')} · ${when} · ${esc(t.status||'')}</small></div><b class="${credit?'credit':'debit'}">${sign}${money(t.amount)}</b></div>`;}).join('')||'<div class="hassan-tx-empty">No recent transactions yet.</div>';}
}
