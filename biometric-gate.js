(function(){
  ...
  async function enroll(user){
    ...
    const cred=await navigator.credentials.create({publicKey:registrationOptions(options)});
    if(!cred)throw new Error('Biometric setup was cancelled.');
    const result=await invoke('registration-verify',{response:registrationResponse(cred)});
    if(!result?.verified)throw new Error('The server could not verify this biometric credential.');
    localStorage.setItem(key(user.id),'server-verified');return true;
  }
  async function verify(user,purpose='authentication'){
    if(!supported())throw new Error('Biometric authentication is not supported on this device or browser.');
    ...
    const result=await invoke('authentication-verify',{purpose,response:authenticationResponse(cred)});
  ...
  async function hasCredential(user){
    ...
    try{await invoke('authentication-options',{purpose:'authentication'});return true}catch{return false}
  }
  async function sensitiveGate(){const u=await getUser();if(!u)throw new Error('Your session has expired. Please log in again.');if(!await hasCredential(u))throw new Error('Biometric confirmation is not enabled. Enable Face ID / fingerprint from your member dashboard first.');return verify(u,'sensitive')}
  ...
  document.addEventListener('submit',async e=>{
    ...
    try{
      ...
      if(await hasCredential(user)){if(button)button.textContent='VERIFYING BIOMETRIC…';await verify(user,'authentication')}
      else if(supported()){
      ...
      if(message){message.textContent='Login successful. Opening your member portal…';message.className='message show';message.style.background='#e7f6ec';message.style.color='#146b31'}
