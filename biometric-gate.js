(function(){
  ...
  async function verify(user,purpose='authentication',sensitiveAction=null,requestHash=null){
    ...
    const options=await invoke('authentication-options',{purpose});
    if(!options?.challenge)throw new Error('Biometric confirmation is not enabled. Enable Face ID / fingerprint from Security Center first.');
    const cred=await navigator.credentials.get({publicKey:authenticationOptions(options),mediation:'optional'});
    if(!cred)throw new Error('Biometric verification was cancelled.');
    const result=await invoke('authentication-verify',{purpose,sensitive_action:sensitiveAction||undefined,request_hash:requestHash||undefined,response:authenticationResponse(cred)});
    if(!result?.verified)throw new Error('Biometric verification failed.');
    localStorage.setItem(key(user.id),'server-verified');
