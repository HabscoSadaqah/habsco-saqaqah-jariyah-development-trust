(function(){
  ...
  async function invoke(action,body={}){
    ...
    let session=null;
    try{const r=await c.auth.getSession();session=r?.data?.session||null;if(!session?.access_token){const refreshed=await c.auth.refreshSession();session=refreshed?.data?.session||null}}catch(e){throw new Error('Your session could not be verified. Please log in again.')}
    if(!session?.access_token)throw new Error('Your session has expired. Please log in again.');
    const {data,error}=await c.functions.invoke('verify-biometric',{body:{action,...body},headers:{Authorization:`Bearer ${session.access_token}`}});
    if(error){let detail='';try{if(error.context&&typeof error.context.json==='function'){const x=await error.context.json();detail=x?.error||x?.message||''}}catch{}const raw=String(error.message||'');if(/401|unauthorized|jwt/i.test(raw+detail))throw new Error('Your login session is no longer valid. Please log in again and try again.');if(/failed to send|fetch|network|cors|relay/i.test(raw+detail))throw new Error('Unable to reach biometric security service. Please refresh the page and try again.');throw new Error(detail||raw||'Biometric security service is unavailable.')}if(data?.error)throw new Error(data.error);return data
  }
  ...
  function requestPayload(action,form){
    ...
    if(action==='member_start_utility_transaction'){
      ...
      const provider=(val('provider')||val('network')||val('disco')).trim().toUpperCase();
      const phone=(val('phone')||val('phone_number')).trim();
      const receiver=(val('receiver')||phone||val('meter_number')).trim();
      const variation=val('variation_code').trim()||val('code').trim()||'';
      const pkg=val('package').trim()||val('package_code').trim()||variation||'';
      const meterNumber=(val('meter_number')||val('receiver')).trim();
      const metadata={
        action:a,
        provider:provider,
        phone:phone,
        receiver:receiver,
        variation_code:variation||null,
        package:pkg||null,
        meter_type:val('meter_type').trim()||null,
  ...
  async function enroll(user){if(!supported())throw new Error('Biometric authentication is not supported on this device or browser.');if(!user)throw new Error('Please log in first.');const options=await invoke('registration-options');const cred=await navigator.credentials.create({publicKey:registrationOptions(options)});if(!cred)throw new Error('Biometric setup was cancelled.');const result=await invoke('registration-verify',{response:registrationResponse(cred)});if(!result?.verified)throw new Error('The server could not verify this biometric credential.');localStorage.setItem(key(user.id),'server-verified');return true}
  async function verify(user,purpose='authentication',sensitiveAction=null,requestHash=null){if(!supported())throw new Error('Biometric authentication is not supported on this device or browser.');const options=await invoke('authentication-options',{purpose});if(!options?.challenge)throw new Error('Biometric confirmation is not enabled. Enable Face ID / fingerprint from Security Center first.');const cred=await navigator.credentials.get({publicKey:authenticationOptions(options),mediation:'optional'});if(!cred)throw new Error('Biometric verification was cancelled.');const result=await invoke('authentication-verify',{purpose,sensitive_action:sensitiveAction||undefined,request_hash:requestHash||undefined,response:authenticationResponse(cred)});if(!result?.verified)throw new Error('Biometric verification failed.');localStorage.setItem(key(user.id),'server-verified');return result.authorization_token||true}
  async function hasCredential(user){return !!(user&&supported()&&localStorage.getItem(key(user.id))==='server-verified')}
  async function sensitiveGate(action=pageAction(),form=null){if(sensitiveGateInFlight)return sensitiveGateInFlight;sensitiveGateInFlight=(async()=>{const u=await getUser();if(!u)throw new Error('Your session has expired. Please log in again.');if(!action)throw new Error('Biometric confirmation is not required for this action.');const payload=requestPayload(action,form);if(!payload)throw new Error('Unable to bind biometric confirmation to this request. Please refresh and try again.');const requestHash=await sha256(canonical(payload));const token=await verify(u,'sensitive',action,requestHash);if(typeof token!=='string')throw new Error('Server did not issue a biometric authorization.');window.__hfBioAuthorizationToken=token;return token})();try{return await sensitiveGateInFlight}finally{sensitiveGateInFlight=null}}
  ...
  function addSecurityBiometricCard(){const panel=document.getElementById('hfSecurityCenter');if(!panel||document.getElementById('hfBiometricCard'))return false;const row=panel.querySelector('.hf-security-row');if(!row)return false;const b=document.createElement('button');b.type='button';b.id='hfBiometricCard';b.innerHTML='<span class="ico">◉</span><span><span class="ttl">Face ID / Fingerprint</span><span class="sub">Enable biometric confirmation</span></span>';row.appendChild(b);b.onclick=async()=>{const original=b.innerHTML;try{b.disabled=true;b.querySelector('.sub').textContent='Waiting for device verification…';await window.hfEnrollBiometric();b.querySelector('.ttl').textContent='Biometric Enabled';b.querySelector('.sub').textContent='Face ID / fingerprint is ready';const m=document.getElementById('hfSecurityMsg');if(m){m.textContent='Biometric confirmation is enabled. You can now confirm sensitive transactions with Face ID or fingerprint.';m.className='hf-security-msg show info'}}catch(err){b.innerHTML=original;const m=document.getElementById('hfSecurityMsg');if(m){m.textContent=err.message||'Unable to enable biometrics.';m.className='hf-security-msg show error'}}finally{b.disabled=false}};return true}
